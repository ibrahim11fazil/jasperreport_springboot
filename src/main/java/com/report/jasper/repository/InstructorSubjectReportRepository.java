...
extends JpaRepository<InstructorSubjectReport,Integer> 
{
	StringBuilder strQuery = new StringBuilder();
	  ...
	  @Query( value = "select * from TAC_INSTRUCTOR_SUBJECT_VIEW  \n" + 
		  		"where (UPPER(NAME) like UPPER('%'||:name||'%') or trim(:name)  is null)\n" + 
		  		"AND (UPPER(TYPE) like UPPER('%'||:type||'%') or trim(:type)  is null)\n" +
		  		"AND (UPPER(JOB_ID)  like UPPER('%'||:jobId||'%')  or trim(:jobId) is   null)\n" +  
				"AND ( UPPER(ORGANIZATION)  like UPPER('%'||:organisation||'%')   or trim(:organisation) is   null)\n" +  
				"AND ( UPPER(JOB_TITLE) like UPPER('%'||:jobTitle||'%')  or trim(:jobTitle) is   null)\n" + 
				"AND ( UPPER(COMPANY_NAME) like UPPER('%'||:companyName||'%')  or trim(:companyName) is   null)\n" +  
				"AND ( UPPER(QID)  like UPPER('%'||:qid||'%')  or trim(:qid) is   null)\n" +  
				"AND ( UPPER(COURSE)   like UPPER('%'||:course||'%')  or trim(:course) is  null)\n" + 
				"AND ( UPPER(PRIORITY)	   like UPPER('%'||:priority||'%')  or  trim(:priority) is  null) ",nativeQuery = true)
